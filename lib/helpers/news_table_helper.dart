  ...
  const InsertNews({required this.newsId, required this.header, required this.imageUrl, required this.description, required this.author, required this.time });
   ...
   Future<void> addData() async{
       ...
       NewsItem? item = await _database.selectOneNews(newsId);
            if(item == null)
            {
              int result = await _database.addNews(NewsItemsCompanion(id: drift.Value(newsId),title:  drift.Value(header),description: drift.Value(description),author: drift.Value(author),content: drift.Value(description),url: const drift.Value('www.testurl.com'),urlToImage: drift.Value(imageUrl),),);
